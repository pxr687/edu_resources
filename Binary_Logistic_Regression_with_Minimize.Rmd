...
Now we start to get more formal, and develop of model of in which we predict the `appetite_dummy` values from the  `Hemoglobin` values.
...
Why not use this same technique for our case?  After all, the `appetite_dummy` values are just numbers (0 and 1), as are our `Hemoglobin` values.
...
value for each observation in the dataset. So if you have 200 observations,
there will be 200 scores on the predictor variable, and 200 predictions, in
`predicted`. By contrast, the slope and intercept are single values, defining
the line.

We use `minimize` to find the values of the slope and intercept which give the
best predictions.  So far, we have almost invariably defined the *best* values
for slope and intercept as the values that give the smallest sum of the
...
Let us grab the `ss_error_for_minimize` function from the [Using minimize
page](../mean-slopes/using_minimize.html), but adapt it for the current
example
...
is *cheap* - i.e. the value is small.  When we have a bad pair in `c_s`, our
function is *expensive* - the value is large.

If the value from `ss_any_line` is large, it means the line we are fitting
does not fit the data well at all. The purpose of linear regression is to find
the line which leads to the smallest cost.  In our case, the cost is the sum of the squared prediction errors.

Let's use linear regression on the current example.  From looking at our plot above, we start with a guess of -1 for the intercept, and 10 for the slope.
...
min_lin_reg = minimize(ss_any_line, [-1, 10], args=(predictor, outcome))
...
# Show them.
print('Best intercept', lin_reg_intercept)
print('Best slope', lin_reg_slope)
```
